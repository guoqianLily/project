<template>
  <div id="policyProfeedback" class="listPage">
    ...
    <div
      class="listBox"
      style="height:63%; width:calc(100% - 52px);padding-left:52px;"
    >
      <div class="tableBox" style="height:100%;">
        <el-table
          ...
          :data="tableData"
          :border="tableData && tableData.length > 0 ? true : false"
        ...
        >
          ...
          <el-table-column prop="nextPolicyContext" label="下周预算">
            <template slot-scope="scope">
              <div
                :style="scope.row.nextPolicyContext ? '' : 'text-align:center;'"
              >
                <span
                  v-html="
                    scope.row.nextPolicyContext
                      ? scope.row.nextPolicyContext
                      : '/'
                  "
                ></span>
          ...
          >
            <template slot-scope="scope">
              <div
                :style="scope.row.proPolicyTypeName ? '' : 'text-align:center;'"
              >
                <span
                  v-html="
    ...
    >
      ...
      >
        ...
        >
          <quillEditor
            :content="addForm.nextPolicyContext"
            style="height:120px;"
        ...
        >
          <el-select
            v-model="addForm.weekproProgressType"
            placeholder="请选择"
            @change="getVal"
          >
            <el-option
...
import { getBtnsPermissionsData } from "../../services/Manage/postManage.js";
import { getMonthMessage } from "../../services/declaresth";
...
export default {
  ...
  mounted() {
    ...
    this.nowTimeValue =
      ...
      "周";
    this.monthVal =
      newValDate.substring(0, 4) + "-" + newValDate.substring(4, 6);
    ...
    handleEdit(index, row) {
      ...
      this.$nextTick(() => {
        this.addForm = {
          ...
          weekproProgressType: row.weekproProgressType
            ? row.weekproProgressType
            : " ", //政策进展分类
          weekId: row.weekId
        };
          ...
          if (_that.FormType == "updateForm") {
            if (this.addForm.weekId) {
              let data = {
                ...
                month: this.monthVal,
                week: this.weekVal,
                content: this.addForm.weekContent
                  ? this.addForm.weekContent
                  : "", //本周进展内容
                nextWeekTargetContent: this.addForm.nextPolicyContext
                  ? this.addForm.nextPolicyContext
                  : "",
                proProgressType: this.addForm.weekproProgressType
                  ? this.addForm.weekproProgressType
                  : "" //政策进展分类
              };
