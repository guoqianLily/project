<template>
    <div class="postmanage" style="width:100%;height:100%;" v-loading="loading" element-loading-text="拼命加载中"
        ...
        <div class="postButtons">
            <label>岗位:</label><el-input v-model="titleSearchStr" placeholder="名称"></el-input>
            <el-button type="primary" icon="el-icon-search" size="mini" @click="titleNameLike = titleSearchStr">查询</el-button>
        ...
        <div class="postTable">
            <el-table :data="tableDataFilter" :height="heightItem" :max-height="heightItem" border
                :header-cell-style="{padding:'8px 0'}" :cell-style="{padding:'5px 0'}">
                <template slot-scope="rowData" v-if="rowData.isMatched!=false">
                ...
                <el-table-column prop="hierarchy" label="岗位类别"></el-table-column>
            ...
            <el-dialog title="岗位信息" :visible.sync="dialogFormVisible" :close-on-click-modal="false"
                @close="cancelHandel">
                <el-form :label-position="labelPosition" ref="postManageFormLog" :inline="true" :model="form"
                    ...
                    <el-form-item label="岗位类别" prop="hierarchy">
            ...
            tableDataFilter:{
                get:function(){
                    return this.tableData.filter((item) => {
                        return item.titleName.match(this.titleNameLike)
                    })
                }
            }
        ...
        methods: {
            ...
            handleDelete(index, row) {
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    deletejudgeDictionaryByChild(row.id).then((res) => { // 验证
                        if (res.success) {
                            deleterjudgeDictionaryByUser(row.id).then(data => { // 验证
                                if (data.success) {
                                    deletepostManList(row.id).then((
                                    result) => {
                                        if (result.success) {
