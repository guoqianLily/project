  ...
  <div class="managePage" v-loading="loading" element-loading-text="拼命加载中" element-loading-background="#000">
    <el-container>
      <el-aside width="200" style="background-color:#000">
        ...
        <!--左侧菜单组件-->
        <el-menu :default-active="isId" @select="handleSelect" class="el-menu-vertical-demo" 
      ...
      <el-container>
        <el-main style="height:100%;">
          <el-tabs v-model="editableTabsValue" type="card" closable @edit="handleTabsEdit" style="height:40px;"
            @tab-click="tabsClick">
...
<script>
  ...
  } from '../services/Manage/postManage'
  export default {
    data() {
      return {
        totalList: [],
            ...
            tabs.forEach((tab, index) => {
              if (tab.name === targetName) {
                ...
                if (nextTab) {
                  ...
                  console.log(this.editableTabs[this.editableTabs.length - 1])
                  this.$router.push({
                    name: this.editableTabs[this.editableTabs.length - 1].path
                  })
                }
              }
            });
...
<style>
  ...
  .managePage .navMenu .el-menu-item:hover {
